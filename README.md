...
文档朗读系统支持本地解析 DOCX、TXT 和 Markdown，并通过服务端 MinerU 接口解析 PDF、图片及其他 Office 文档。原文件、解析正文和图片资产保存在浏览器 IndexedDB 中，可按文档重新打开或清理。

部署时需为项目连接一个公开读取的 Vercel Blob 存储。待解析文件由浏览器直传 Blob，MinerU 完成解析后自动删除临时文件；解析结果仍只保存在当前浏览器的 IndexedDB。

## ✨ 核心特性

- **多格式文档解析**：DOCX 在浏览器内由 `mammoth.js` 解析；PDF、图片及其他 Office 文档由服务端代理 MinerU 精准解析。
- **本地文档库**：原文件、Markdown、图片等解析资产和朗读缓存保存在 IndexedDB，可按文档打开或清理。
- **智能句子分割与长文本处理**：
...
复制 `.env.example` 为 `.env.local`，填写服务端使用的 `MINERU_API_TOKEN`。该 Token 只由 Next.js Route Handler 读取，不会发送到浏览器。
