...
- **智能句子分割与长文本处理**：
  - 精准拆分中英文字句。
  - 对于超长句子（>150字符）支持智能按标点符号断句分块加载，确保 TTS 请求稳定不超时。
- **极致的视觉反馈与动画设计**：
...
1. **多重预加载策略 (Preloading & Caching)**:
   在 `app/page.tsx` 中，音频管理采用了维护 `audioCache` Map 的策略。当播放当前句子时，系统会自动向后预取最多 5 个句子的音频。遇到过长句子则进行分块（Chunking）并发请求并缓存，确保朗读完全不卡顿。
2. **纯客户端渲染**:
   涉及 Web Audio、`localStorage` 和 File API，`page.tsx` 底层被 `next/dynamic` 包裹以禁用 SSR，保证应用完美兼容并且无需担心服务端变量不匹配。
3. **安全与清理**:
   使用 `mammoth.js` 将 Word 转为粗略 HTML 后，经过 `DOMPurify` 严格过滤防 XSS，随后将 HTML 喂入自定义的 `textProcessor.ts` 内重建 DOM 树定位 TextNode 进而精细地插入 `.current-reading` 高亮。
